...
@property (nonatomic,retain) NSMutableDictionary *muD_allSectionValues/*所有section里边的值(每个section里有几个数据对象),每个key是section名字,用于section>1&&需要A--#排序显示时*/,*muD_allSectionValueCopy/*搜索前把[muD_allSectionValues mutableDeepCopy],搜素完成后重置tbv的数据源*/,*muD_dicfferIndexForCellView;
